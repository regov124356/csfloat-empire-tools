...
(function () {
    ...
    function setupItemClicker() {
        document.addEventListener('click', (e) => {
            // Celujemy w nagłówek karty przedmiotu
            ...
            if (nameEl) {
                ...
                // Szukamy czy w subtekście jest znany Wear (stan zużycia)
                let foundWear = WEARS.find(wear => subtext.includes(wear));

                if (foundWear) {
                    // Dodajemy stan w nawiasie - format idealny pod wyszukiwarkę Empire
                ...
                navigator.clipboard.writeText(finalQuery).then(() => {
                    ...
                    // Opcjonalne: Wizualne potwierdzenie na elemencie
    ...
    // ==========================================
    // STYLE I UI (Przelicznik cen pozostaje bez zmian)
    // ==========================================
    function injectStyles() {
        const style = document.createElement('style');
        style.innerHTML = `
            mat-card > div > div:first-child:has(app-item-name) {
                cursor: copy !important; /* Zmiana kursora na ikonę kopiowania */
    ...
    function handleInputConversion() {
        ...
        const parent = input.closest('.mat-mdc-form-field-infix') || input.parentElement;
        let resSpan = document.createElement("div");
        resSpan.style = "font-size:12px; margin-top:4px;";
        parent.appendChild(resSpan);
        const recalc = () => {
            const rate = parseFloat(localStorage.getItem(STORAGE_KEY));
            const price = parseFloat(input.value.replace(/,/g, "").trim());
            if (!rate || isNaN(price)) { resSpan.innerHTML = ""; return; }
            const raw = price / (rate / 10);
            const withFee = raw * 1.05;
            resSpan.innerHTML = `Empire: <span style="color:#f44336">${raw.toFixed(2)}c</span> | <span style="color:#4caf50">${withFee.toFixed(2)}c</span>`;
        };
    ...
    function init() {
        injectStyles();
        createInput();
        setupItemClicker(); // Funkcja teraz aktywna
